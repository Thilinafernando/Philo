...
/*   Updated: 2025/06/06 22:21:46 by tkurukul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_philo_info(t_info *info)
{
	...
	while (++i < info->num_philo)
	{
		...
		if (pthread_mutex_init(&info->philo[i].meal_mutex, NULL) != 0)
			return (err_parsing("Mutex init failed", -42),
...
void	fork_init(t_info *info, int i)
{
...
int	init_structpt2(t_info *info)
{
	int	i;
